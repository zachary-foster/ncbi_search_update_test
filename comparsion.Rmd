...
The two functions differ in how they handle no results or invalid taxon names. ncbi_search2 returns a 0-row dataframe if no sequences are found. This allows for the results of sucessful and unsucessful searches to be `rbind`ed together without further modifications.
...
```{r}
query <- c("pythium", "phytophthora")
x <- ncbi_search(id = as.character(1:3), limit = 3)
x
y <- ncbi_search2(id = 1:3, limit = 3)
y
all.equal(x, y)
```
...
When there is an accession number prefix other than those that are removed, only the prefix is retured. 
...
In the curent `ncbi_search` the `getrelated` option seems to work by extracting the genus name of a latin binomial name (it assumes the input is a species).
...
Since the `taxa` argument is converted to a uid, this change also means that the `getrelated` option will work wether an uid or a taxon name is given. 
...
If a taxon name given has no uid and is a species binomial, the original implemenation of `getrelated` is used. 
